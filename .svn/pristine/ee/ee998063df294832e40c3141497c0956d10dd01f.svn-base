...
//        $this->request->allowMethod('post', 'delete');
    ...
    public function add($case_id=null){
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            if(!$this->Caseassessment->saveAll($this->request->data, array("deep" => true))){
                $error = true;
            }
            if(!$this->Caseassessment->Casemanagementform->saveField('form_id', $this->Caseassessment->id)){
                $error = true;
            }

            $this->Casemanagement->id = $case_id;
            if(!$this->Casemanagement->saveField('casenature_id', $this->request->data['Caseassessment']['casenature_id'])){
                $error = true;
            }
            if(!$this->Casemanagement->saveField('casetype_id', $this->request->data['Caseassessment']['casetype_id'])){
                $error = true;
            }

            if($this->request->data['Caseassessment']['nextreviewdate']){
                if(!$this->Casemanagement->saveField('nextreviewdate', $this->request->data['Caseassessment']['nextreviewdate'])){
                    $error = true;
                }
            }
