...
//                    }
        ...
        if(!empty($input['identity'])){
            $idfilter_ids = $this->Membership->Member->find("list", array(
                "conditions"=>array(
                    "Member.identityhashkey Like"=>"%".$this->Membership->Member->datahash(strtoupper(trim($input['identity'])))."%"
                ),
                "fields"=>array("id", "id")
            ));
            if(!empty($options['conditions']['Member.id'])){
                $options['conditions']['Membership.member_id'] = $idfilter_ids;
            }else{
                $options['conditions']['Membership.member_id'] = $idfilter_ids;
            }
        ...
        if(!empty($input['name'])){
            $idfilter_ids = $this->Membership->Member->find("list", array(
                "conditions"=>array(
                    "Member.displayname Like"=>"%".trim($input['name'])."%"
                ),
                "fields"=>array("id", "id")
            ));
            if(!empty($options['conditions']['Member.id'])){
        ...
        if(!empty($input['tel'])){
            $idfilter_ids = $this->Membership->Member->find("list", array(
                "conditions"=>array(
                    "AND"=>array(
                        "OR"=>array(
                            "Member.contact_tel_home Like"=>"%".trim($input['tel'])."%",
                            "Member.contact_tel_mobile Like"=>"%".trim($input['tel'])."%"
                        )
                    )
                ),
                "fields"=>array("id", "id")
        ...
        if(!empty($input['membercard'])){
            $options['conditions']['Membership.membercard'] = trim($input['membercard']);
        }
        ...
        foreach ( $result as $doc ) {
            ...
            $doc['Memberoutput']['action'] = " ".$html->link('<button class="btn btn-info btn-sm">選擇</button>', "javascript:void(0)", array('class' =>'selectmember', 'data-identity'=>h($doc['Member']['identity']), 'data-displayname'=>h($doc['Member']['displayname']), 'data-msid'=>$doc['Membership']['id'], 'data-mid'=>$doc['Member']['id'],  'escape' => false));
