...
//            $code = $rs['Eventproposaltype']['prefix'].$id;
    ...
    public function beforeSave($options = array()) {
        ...
        $stringallow = '<span contenteditable="false" ><input type="checkbox"></span>';
        $stringreplace = "__CHECKBOX__";
...
//        Configure::write('debug', 2);
        if (!empty($this->data[$this->alias]['proposal_content'])) {
//            $this->data[$this->alias]['volunteerdetail'] = str_replace($stringallow,  $stringreplace, $this->data[$this->alias]['volunteerdetail']);
//            debug($this->data);
//            $this->data[$this->alias]['volunteerdetail'] = $purifier->purify($this->data[$this->alias]['volunteerdetail']);
//            $this->data[$this->alias]['volunteerdetail'] = str_replace($stringreplace, $stringallow, $this->data[$this->alias]['volunteerdetail']);
            $this->data[$this->alias]['proposal_content'] = base64_encode($this->data[$this->alias]['proposal_content']);
...
//        exit();
    ...
    public function afterFind($results, $primary = false) {
        foreach ($results as $key => $val) {
            if(isset($results[$key][$this->alias]['proposal_content'])){
                $results[$key][$this->alias]['proposal_content'] = base64_decode($results[$key][$this->alias]['proposal_content']);
            }
