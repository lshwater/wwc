...
<div class="page-header">
    ...
    <!-- Page header, center on sm  all screens -->
        <h1 class="col-xs-12 text-left-sm flashMessage">
               <i class="fa fa-bars page-header-icon"></i>&nbsp;&nbsp;管理參加者名單
        </h1>

        <div class="pull-right col-sm-auto">
            <?php echo $this->Html->link('出席情況', array('action' => 'viewallattendant' , $activity['Activity']['id']), array('escape'=>false, 'class'=>'btn btn-primary btn-labeled')); ?>
            <?php echo $this->Html->link('點名表', array('action' => 'exportattendentsheet' , $activity['Activity']['id']), array('escape'=>false, 'class'=>'btn btn-primary btn-labeled')); ?>
        </div>
    </div>
</div>
