...
class CasemanagementsController extends AppController
{
    ...
    public function schedule($selected = null){
    ...
    public function myschedule($selected = null){
        $this->set('casenatures', $this->Casemanagement->Casenature->find('list', array('conditions' => array('active' => 1))));
        $this->set('casetypes', $this->Casemanagement->Casetype->find('list', array('conditions' => array('active' => 1))));
        $this->set('servicerecords', $this->Casemanagement->Casemanagementform->Servicerecord->Servicerecordtype->find('list', array('conditions' => array('active' => 1))));
        $this->set('users',$this->Casemanagement->User->find('list',array('fields'=>array('id', 'username'))));
...
//        configure::Write('debug',2);
        ...
        if($this->request->is('post')){
            ...
            $servicerecords_list = $this->Casemanagement->Casemanagementform->Servicerecord->Servicerecordtype->find('all', array('conditions' => array('active' => 1)));
            $servicerecords_list = Set::combine($servicerecords_list, '{n}.Servicerecordtype.id', '{n}.Servicerecordtype');
            ...
            foreach($result['Casemanagementform'] as $k=>$row){
                ...
                $event = array();
                $event['id'] = $row['id'];
                $event['title'] = $servicerecords_list[$row['servicerecordtype_id']]['name'];
                ...
                $event['color'] = $servicerecords_list[$row['servicerecordtype_id']]['color_code'];
                $event['editable'] = true;
    ...
    public function ajax_myschedule_list(){
//        configure::Write('debug',2);
        $this->autoRender = FALSE;
        ...
        if($this->request->is('post')){
            ...
            $this->Casemanagement->Casemanagementform->Servicerecord->Behaviors->load('Containable');

            $options['conditions'] = array(
                'Servicerecord.service_provider_id'=>$this->Auth->user('id'),
                'Servicerecord.date BETWEEN ? AND ?'=>array(date('Y-m-d',strtotime($start)), date('Y-m-d',strtotime($end))),

            );

            $options['contain'] = array(
                'Casemanagementform.Casemanagement.Member',
            ...
            $result = $this->Casemanagement->Casemanagementform->Servicerecord->find('all', $options);

            foreach($result as $k=>$row){
                $event = array();

                $event['title'] = $servicerecords_list[$row['Servicerecord']['servicerecordtype_id']]['name'];
                $event['title'] .= " / ".$row['Casemanagementform']['Casemanagement']['Member']['displayname'];
                $event['title'] .= " / ".$row['Servicerecord']['start_time']." - ".$row['Servicerecord']['end_time'];

                $event['start'] = $row['Servicerecord']['date']." ".$row['Servicerecord']['start_time'];
                $event['end'] = $row['Servicerecord']['date']." ".$row['Servicerecord']['end_time'];
                $event['className'] = "event";
                $event['color'] = $servicerecords_list[$row['Servicerecord']['servicerecordtype_id']]['color_code'];
                ...
                $event['description'] = $html->link('<button class="btn btn-sm btn-info">查看</button>', array('controller' => 'casemanagements', 'action' => 'view', $row['Casemanagementform']['Casemanagement']['id']), array('escape'=>false))." ";
