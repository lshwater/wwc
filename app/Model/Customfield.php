...
//                debug($rst);
                ...
                foreach($rst as $item){
                    $select['value'][] = $item[$dbtable]['value'];
                    $select['value_text'] .= $item[$dbtable]['value_text']. ",";
                }
                $select['value_text'] = substr($select['value_text'], 0, -1);

                $fields[$k] = array_merge($fields[$k], $select);
            }else{
    ...
    public function savecusfield_by_field_id($field_id, $item_id, $value){
        ...
        if(!isset($field_id)|| !isset($item_id)){

            $rst['errormsg'] = "Invalid input #".$field_id;
            return $rst;
        }
...
//        configure::write('debug',2);
    ...
    public function searchcusfield_by_alias($model_id, $alias, $value){

        if(!isset($model_id) ||  !isset($alias)|| !isset($value)){
            return false;
        }
