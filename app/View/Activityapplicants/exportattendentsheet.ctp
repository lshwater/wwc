...
for($i = 0; $i < sizeof($activityapplicants); $i++){
    //print header
    if($i % $printprepage == 0){
        if($pageprinting){
            //close that page
        ...
        $pageprinting = true;
        $this->PhpExcel->loadWorksheet(APP.'Files/exportattendentsheet.xls');
    ...
    $printname = $activityapplicants[$i]['Activityapplicant']['c_name'];
    if(empty($printname)){
        $printname = $activityapplicants[$i]['Activityapplicant']['e_name'];
    }
    ...
    $this->PhpExcel->addTableRow(
        array(
            $i+1,
            $printname,
            $activityapplicants[$i]['Member']['code'],
            '','','','','','','','','','',
            $activityapplicants[$i]['Activityapplicant']['tel'],
            $activityapplicants[$i]['Activityapplication']['payment_code'],
            $activityapplicants[$i]['Activityapplicant']['cost']
        )
    );
