...
<ul class="breadcrumb">
    <li>
        <?=$this->Html->link(__("管理參加者名單"), array("action"=>"management", $activity['Activity']['id']))?>
    </li>
...
<div class="panel">
    <div class="panel-heading">
        <span class="panel-title"> <strong><?=h($activity['Activity']['name'])?> ( <?=h($activity['Activity']['activity_code'])?> )</strong></span>
    </div>
                    ...
                    <table cellspacing="0" class="table table-striped nowrap" id="jq-datatables" width="100%">
                        ...
                        <tbody>
                            ...
                            if(!empty($activityapplicants)){
                                foreach($activityapplicants as $applicant){
                            ?>
                                    <tr>
                                        <td></td>
                                        <td>
                                            <?php
                                            if(!empty($applicant['Activityapplicant']['c_name'])){
                                                echo h($applicant['Activityapplicant']['c_name']);
                                            }else{
                                                echo h($applicant['Activityapplicant']['e_name']);
                                            }
                                            ?>
                                        </td>
                                        <?
                                        foreach($activity['Activitysession'] as $k=>$activitysession){
                                            ...
                                            ?>
                                            <td>
                                                <?if(!empty($applicant['Activityattendant'][$activitysession['id']]['Attendant']['name'])){?>
                                                    <span class='<?=h($applicant['Activityattendant'][$activitysession['id']]['Attendant']['labelclass'])?>'>
                                                        <?php
                                                            echo h($applicant['Activityattendant'][$activitysession['id']]['Attendant']['name']);
                                                        ?>
                                                    </span>
                                                <?}else{
                                                    echo "NIL";
